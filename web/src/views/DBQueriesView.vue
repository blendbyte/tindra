...
const { perf, hours, timeseries, isLoading, isError, refetch, sortCol, toggleSort, sortIcon, filtered, noData, search, selectedRow } =
  useSpanTable({ endpoint: 'db', queryKeyPrefix: 'span-db' })
</script>
...
<template>
  <div class="page">
    ...
    <div v-if="isError" class="txerror">
      ...
      <span>Couldn't load query data. Check your connection and try again.</span>
    ...
    <div v-if="timeseries && timeseries.buckets.length > 0" class="txcharts">
      <div class="txchart-panel">
        <div class="txchart-panel__label">QPM</div>
        <TimeseriesChart
          :times="timeseries.buckets.map(b => b.time)"
          :series="[{ id: 'count', label: 'QPM', type: 'bar', values: timeseries.buckets.map(b => b.count) }]"
    ...
    <div v-if="noData" class="empty-state">
      <div class="empty-state__card">
        <div class="empty-state__icon empty-state__icon--ok"><Icon name="database" :size="28" /></div>
        <h2 class="empty-state__title">No database spans in this window</h2>
        <p class="empty-state__body">Instrument your app using a Sentry SDK with database query tracking enabled.</p>
      </div>
    </div>
        ...
        <table class="perf-table">
          <thead>
            <tr>
              ...
              <th class="perf-table__num"><button class="col-sort" :class="{ 'col-sort--active': sortCol === 'rate' }" @click="toggleSort('rate')">QPM <em class="col-sort__icon">{{ sortIcon('rate') }}</em></button></th>
          ...
          <tbody>
            ...
            <tr v-if="!isLoading && filtered.length === 0 && search">
              <td colspan="7" style="padding: 24px; text-align: center; color: var(--text-3); font-size: var(--text-sm)">No queries match "{{ search }}"</td>
            </tr>
          </tbody>
        </table>
      </div>
