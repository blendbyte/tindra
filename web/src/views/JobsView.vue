...
const { perf, hours, timeseries, isLoading, isError, refetch, sortCol, toggleSort, sortIcon, filtered, noData, search, selectedRow } =
  useSpanTable({ endpoint: 'jobs', queryKeyPrefix: 'span-jobs' })
</script>
  ...
  <div class="page">
    ...
    <div class="filterbar">
      ...
      <div class="filterbar__search">
        ...
        <input v-model="search" placeholder="Filter jobs…" aria-label="Filter jobs" />
      </div>
    </div>

    <div v-if="isError" class="txerror">
      <Icon name="alert-circle" :size="16" class="txerror__icon" />
      <span>Couldn't load job data. Check your connection and try again.</span>
    ...
    <div v-if="!isError && timeseries && timeseries.buckets.length > 0" class="txcharts">
      <div class="txchart-panel">
        ...
        <TimeseriesChart
          ...
          :series="[{ id: 'count', label: 'TPM', type: 'bar', values: timeseries.buckets.map(b => b.count) }]"
    ...
    <div v-if="noData" class="empty-state">
      <div class="empty-state__card">
        <div class="empty-state__icon empty-state__icon--ok"><Icon name="clock" :size="28" /></div>
        <h2 class="empty-state__title">No job or queue spans in this window</h2>
        <p class="empty-state__body">Instrument your workers using a Sentry SDK with task/queue integration.</p>
      </div>
    </div>
        ...
        <table class="perf-table">
          <thead>
            <tr>
              ...
              <th class="perf-table__num"><button class="col-sort" :class="{ 'col-sort--active': sortCol === 'rate' }" @click="toggleSort('rate')">TPM <em class="col-sort__icon">{{ sortIcon('rate') }}</em></button></th>
          ...
          <tbody>
            ...
            <tr v-if="!isLoading && filtered.length === 0 && search">
              <td colspan="7" style="padding: 24px; text-align: center; color: var(--text-3); font-size: var(--text-sm)">No jobs match "{{ search }}"</td>
            </tr>
          </tbody>
        </table>
      </div>
