...
describe('investigation page recovery', () => {
  ...
  it('resets an invalid project link and keeps the local filters and location', async () => {
    const { wrapper, router, state, requests } = await setup('/performance/queries?project_id=invalid&env=production&window=All&user=alice&name=checkout&op=db.sql#results')
    expect(requests).not.toHaveBeenCalled()
    await wrapper.find('[role="alert"] button').trigger('click')
    await flushPromises()
    expect(state.routeError).toBe('')
    expect(state.projectIds).toEqual([])
    expect(state.environment).toBe('All')
    expect(state.userIdentity).toBe('')
    expect(state.range).toBe('24h')
    expect(router.currentRoute.value.query).toEqual({ project_id: 'all', environment: 'all', user: '', range: '24h', name: 'checkout', op: 'db.sql' })
    expect(router.currentRoute.value.hash).toBe('#results')
    expect(wrapper.find('.perf-subnav__link--active').text()).toBe('Queries')
    expect(wrapper.find('[role="alert"]').exists()).toBe(false)
    expect(requests).toHaveBeenCalledOnce()
