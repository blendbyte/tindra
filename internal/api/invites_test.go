...
func limitedHandler(userLimit int) http.Handler {
	return api.NewRouter(testPool, ingest.NewBuffer(1), nil, nil, nil, nil, nil, false, "", "https://example.com", "", "", 0, 0, 0, userLimit, 0, 0, nil, false, false, nil)
}
...
func TestCreateInvite_withEmailSender_success(t *testing.T) {
	...
	h := api.NewRouter(testPool, ingest.NewBuffer(1), nil, nil, nil, nil, nil, false, "", "https://tindra.example.com", "", "", 0, 0, 0, 0, 0, 0, nil, false, false, nil)
	h.ServeHTTP(rec, req)
	if rec.Code != http.StatusCreated {
		t.Fatalf("expected 201, got %d: %s", rec.Code, rec.Body.String())
	}
